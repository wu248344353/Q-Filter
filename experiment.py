...
def experiment(
    ...
    def get_batch(batch_size=256, max_len=K):
        ...
        for i in range(batch_size):
            ...
            rtg.append(traj['returns_to_go'][si:si + max_len + 1].reshape(1, -1, 1))
            if rtg[-1].shape[1] <= s[-1].shape[1]:  # 后面会用到最后一个数
                rtg[-1] = np.concatenate([rtg[-1], np.zeros((1, 1, 1))], axis=1)
        ...
        s.shape: torch.Size([256, 20, 17]) 
        a.shape: torch.Size([256, 20, 6]) 
        r.shape: torch.Size([256, 20, 1]) 
        target_a.shape: torch.Size([256, 20, 6]) 
        d.shape: torch.Size([256, 20, 1])
        rtg.shape: torch.Size([256, 21, 1]) *****
        timesteps.shape: torch.Size([256, 20]) 
        mask.shape: torch.Size([256, 20])
        """
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_head', type=int, default=8)  # change
    ...
    parser.add_argument("--q_percent", default=0.00, type=float)
