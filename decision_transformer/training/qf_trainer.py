...
class Trainer:
    ...
    def __init__(self,
        ...
        self.log_temperature_optimizer = torch.optim.Adam(
            ...
            lr=1e-4,
            # 尝试增大lr，actor_loss能够稳定，但是性能并未提升
            # lr=4e-3,
            betas=[0.9, 0.999],
        )
    ...
    def train_step(self, log_writer=None, loss_metric={}):
        ...
        with torch.no_grad():
            ...
            _, next_action, _, _, next_action_dist = self.ema_model(
        ...
        rtg_preds, action_preds, state_preds, reward_preds, action_dist_preds = self.actor.forward(
        ...
        log_likelihood = action_dist_preds.log_prob(action_target).sum(axis=2)[action_mask.reshape(action_target.shape[0], action_target.shape[1])].mean()
        # entropy = action_dist_preds.entropy().sum(axis=2)[action_mask.reshape(action_target.shape[0], action_target.shape[1])].mean()
        # action_loss = -(log_likelihood + self.actor.temperature().detach() * entropy)
        action_loss = -log_likelihood
        bc_loss = action_loss
        ...
        # temperature_loss = (
        #         self.actor.temperature() * (entropy - self.actor.target_entropy).detach()
        # )
